...
import { BASE_URL, GCASH_PAYMENT_MODE, GCASH_STATUS_ENDPOINT } from "../config";
...
const transactionStatusInfo = (status) => {
  ...
  if (!normalized) {
    return {
      ...
      badge: GCASH_STATUS_BADGES.default,
      icon: CreditCard,
      normalized
    };
  }
  if (SANDBOX_SUCCESS_STATUSES.has(normalized)) {
    return {
      ...
      badge: GCASH_STATUS_BADGES.success,
      icon: CheckCircle,
      normalized
    };
  }
  if (SANDBOX_FAILED_STATUSES.has(normalized)) {
    return {
      ...
      badge: GCASH_STATUS_BADGES.failed,
      icon: AlertTriangle,
      normalized
    };
  }
  if (SANDBOX_PENDING_STATUSES.has(normalized)) {
    return {
      ...
      badge: GCASH_STATUS_BADGES.pending,
      icon: Clock,
      normalized
    };
  }
  if (SANDBOX_PROCESSING_STATUSES.has(normalized)) {
    return {
      ...
      badge: GCASH_STATUS_BADGES.processing,
      icon: Clock,
      normalized
    };
  }
  return {
    label: `GCash: ${normalized}`,
    badge: GCASH_STATUS_BADGES.default,
    icon: Clock,
    normalized
  };
...
export default function AdminDues() {
  ...
  const syncTransactionStatus = useCallback(
    async (due) => {
      ...
      if (!due.gcash_transaction) {
        showToast("No GCash transaction to sync for this due.", "warning");
        return;
      }
      setSyncingId(due.id);
      try {
        const reference = due.gcash_transaction.reference || "";
        const query = reference
          ? `reference=${encodeURIComponent(reference)}`
          : `due_id=${due.id}`;
        const response = await fetch(`${GCASH_STATUS_ENDPOINT}?${query}&refresh=1`, {
          credentials: "include"
        });
        const payload = await safeJson(response);
        if (!payload.success || !payload.found || !payload.transaction) {
          showToast(payload.message || "GCash transaction not found.", "warning");
          return;
        }
        ...
        setDues((prev) =>
          prev.map((item) =>
            item.id === due.id
              ? {
                  ...item,
                  gcash_transaction: transaction,
                  gcash_reference: transaction.reference || item.gcash_reference
                }
              : item
          )
        );
        showToast("GCash status refreshed.");
        if (transaction.status && isFinalTransactionStatus(transaction.status)) {
          const now = Date.now();
          if (now - lastFinalRefreshRef.current > 3000) {
            ...
            try {
              ...
              console.error("Failed to reload dues after sync", error);
            }
          }
        }
      } catch (error) {
        console.error("Sync GCash status failed", error);
        showToast("Failed to sync GCash status", "error");
      } finally {
        setSyncingId(null);
      }
    },
  ...
  useEffect(() => {
    ...
    const pending = dues
      .filter(
        (due) =>
          due.gcash_transaction &&
          !isFinalTransactionStatus(due.gcash_transaction.status)
      )
      .map((due) => ({
        id: due.id,
    ...
    const pollStatuses = async () => {
      ...
      setDues((prev) => {
        let changed = false;
        const byId = new Map(updates.map((item) => [item.dueId, item]));
        const next = prev.map((item) => {
          const update = byId.get(item.id);
          if (!update) {
            return item;
          }
          const prevStatus = normalizeStatus(item.gcash_transaction?.status);
          const nextStatus = update.normalized;
          const prevCheckout = item.gcash_transaction?.checkout_url || "";
          const nextCheckout = update.transaction.checkout_url || "";
          const prevPaymentId = item.gcash_transaction?.gcash_payment_id || "";
          ...
          return {
            ...item,
            gcash_reference: update.transaction.reference || item.gcash_reference,
      ...
      if (shouldRefresh) {
        ...
        if (now - lastFinalRefreshRef.current > 3000) {
          ...
          try {
            ...
            console.error("Failed to reload dues after polling", error);
          }
        }
      }
    };
  ...
  }, [dues, isSandboxMode, fetchDuesList]);
  ...
  const enrichedDues = useMemo(
    () =>
      dues.map((due) => {
        ...
        const gcashInfo = due.gcash_transaction
          ? transactionStatusInfo(due.gcash_transaction.status)
          : null;
        return {
          ...due,
                        ...
                        <tr key={due.id}>
                          ...
                          <td className="px-4 py-3 text-xs text-gray-600">
                            ...
                            {due.gcash_transaction && (
                              <div className="mt-1 space-y-1">
                                {txnInfo ? (
                                  <span
                                    className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${txnInfo.badge}`}
                                  >
                                    <TxnIcon className="w-3.5 h-3.5" />
                                    {txnInfo.label}
                                  </span>
                                ) : (
                                  <div>
                                    <span className="font-medium">GCash status:</span>{" "}
                                    {due.gcash_transaction.status || "pending"}
                                  </div>
                                )}
                                {due.gcash_transaction.status && txnInfo && (
                                  <div className="text-gray-500">
                                    Raw: {due.gcash_transaction.status}
                                  </div>
                                )}
                                {due.gcash_transaction.checkout_url && (
                                  <div>
                                    <a
                                      href={due.gcash_transaction.checkout_url}
                                      target="_blank"
                                      rel="noreferrer"
                                ...
                                {due.gcash_transaction.updated_at && (
                                  <div className="text-gray-500">
                                    Updated {formatDateTime(due.gcash_transaction.updated_at)}
                                  </div>
                                )}
                              </div>
                            )}
                          ...
                          <td className="px-4 py-3">
                            <ActionsColumn
                              ...
                              onSync={isSandboxMode ? syncTransactionStatus : undefined}
                            />
                          </td>
                        </tr>
                      );
              ...
              sortedDues.map((due) => {
                ...
                return (
                  ...
                  >
                    ...
                    {due.gcash_transaction && (
                      <div className="mt-2 space-y-1 rounded-xl border border-indigo-100 bg-white px-3 py-2 text-xs text-gray-700">
                        {txnInfo ? (
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${txnInfo.badge}`}
                          >
...
function ActionsColumn({
  ...
  const txn = due.gcash_transaction || null;
  const txnInfo = due.gcashInfo || null;
  const TxnIcon = txnInfo?.icon || Clock;
  const showSync = Boolean(onSync && txn);
        ...
        <div className="w-full space-y-1">
          {txnInfo ? (
            <span
              className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${txnInfo.badge}`}
            >
              <TxnIcon className="h-3.5 w-3.5" />
              {txnInfo.label}
            </span>
          ) : (
            <div className="rounded-full bg-indigo-50 px-3 py-1 text-xs text-indigo-700">
              Gateway status: {txn.status || "pending"}
            </div>
          )}
          {txn.updated_at && (
            <div className="text-xs text-gray-500">
              Updated {formatDateTime(txn.updated_at)}
            </div>
          )}
          {txn.checkout_url && (
            <a
          ...
          {showSync && (
            <button
              type="button"
              onClick={() => onSync(due)}
              disabled={syncingId === due.id}
              className={`inline-flex items-center gap-1 rounded-full border border-indigo-200 bg-white text-indigo-600 transition hover:bg-indigo-50 disabled:opacity-60 ${
